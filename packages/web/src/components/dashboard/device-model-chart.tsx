...
import type { DeviceModelBreakdownRow } from "@/lib/device-helpers";
...
interface DeviceModelChartProps {
  data: DeviceModelBreakdownRow[];
  className?: string;
}
...
function ModelTooltip({
  ...
  payload?: Array<{
    ...
    value: number;
    color: string;
    payload?: { model: string; total_tokens: number };
  }>;
  label?: string;
}) {
 ...
 * for a single device. Top 10 models, stacked: input / output / cached.
 */
export function DeviceModelChart({
  data,
  className,
}: DeviceModelChartProps) {
  if (!data.length) {
    return (
      <div
        className={cn(

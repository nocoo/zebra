...
function makePost(): Request {
  return new Request("http://localhost:7020/api/admin/badges/b1/archive", {
    method: "POST",
  });
}
...
describe("POST /api/admin/badges/[id]/archive", () => {
  ...
  beforeEach(async () => {
    ...
    const mod = await import("@/app/api/admin/badges/[id]/archive/route");
    POST = mod.POST;
  });
  ...
  it("returns 400 when badge is already archived", async () => {
    resolveAdmin.mockResolvedValueOnce(ADMIN);
    ...
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe("Badge is already archived");
  });

  it("archives badge successfully", async () => {
    resolveAdmin.mockResolvedValueOnce(ADMIN);
  ...
  it("returns 500 on DB error", async () => {
    ...
    const body = await res.json();
    expect(body.error).toBe("Failed to archive badge");
  });
});
